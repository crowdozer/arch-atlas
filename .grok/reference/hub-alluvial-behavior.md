...
| When intent _does_ change, update this doc **and** goldens in the same change set, with explicit rationale.  | Silently re-home packages, seeds, or reverse rings as a side effect of a polish fix.                  |
...
**Geometry caveat:** an Imports node can still _look_ External-adjacent if Carbon
...
| Through import tree    | **Reserve-then-route:** reserve package budget `pkgReserve = min(arrived, rawPkg)` first; **proportional-split** the remainder (`fileMass = m − pkgReserve`) by parent→child edge weight (same basis as Weight axis - target-loc / exact surface / import-edges) along kept file→file edges to children at `displayDist + 1` (overflow members that share a “+N more” lab still contribute separately then aggregate). Prefer intermediate Kirchhoff when mass can cover both packages and file hops. Equal-split is wrong under target-loc: wide barrels flatten every hop child to 1. |
